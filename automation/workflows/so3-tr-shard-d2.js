export const meta = {
  name: 'so3-tr-shard-d2',
  description: 'SO3 Disc2 한글패치 번역 샤드: args로 받은 배치들을 번역하고 배치별 단독 검증',
  phases: [
...
const DIR = 'C:/Users/Jay/Documents/Codex/2026-07-13/d-3-ps2/work/full_ko_d2'
const WIN = 'C:\\Users\\Jay\\Documents\\Codex\\2026-07-13\\d-3-ps2\\work\\full_ko_d2'
let ids = args
...
const mkPrompt = (i) => (
  `스타오션3 DC(PS2) Disc 2 한글패치 본문 번역. 배치 ${pad(i)}. Disc 2는 게임 후반 스토리다.\n\n` +
  `## 절차 (빠르게, 군더더기 없이)\n` +
  `1. ${D1}/STYLE_GUIDE.md 를 Read (규칙·캐릭터별 말투 — Disc 1과 동일 기준).\n` +
  ...
  `   - glossary + core_names 대역어 준수(활용형 변화는 허용).\n` +
  `   - 배치 내 유닛은 장면 순서 — 앞뒤 흐름 자연스럽게.\n` +
  ...
  `   python "${WIN}\\validate_one_d2.py" ${i}\n` +
  `   errors=0 이 될 때까지 수정 후 재실행(최대 4회). glossary_mismatch 경고는 활용형이면 무시.\n` +
  `5. StructuredOutput: {batch: ${i}, count, errors(최종 0이어야 함), warnings, notes}.`
)

phase('Translate')
const results = await parallel(ids.map((i) => () => agent(
  mkPrompt(i), { label: `d2tr:${pad(i)}`, phase: 'Translate', schema: SCHEMA, model: 'sonnet' }
)))
const clean = results.filter((r) => r && r.errors === 0).length
log(`샤드 완료: ${clean}/${ids.length} 클린`)
return { shard_size: ids.length, clean, batches: ids }
