...
"""Disc-2 single-batch validator (mirrors full_ko\\validate_one.py with d2 paths).
  ...
  python validate_one_r2.py 5   -> tr_batches/batch_0005.json vs tr_out/batch_0005_ko.json
...
BATCHES = os.path.join(D2, "tr_batches_r2")
OUT = os.path.join(D2, "tr_out_r2")
...
def main():
    if len(sys.argv) < 2:
        raise SystemExit("usage: validate_one_r2.py <batch number>")
    n = int(sys.argv[1])
