...
batch and prints a compact verdict, so an agent's self-check loop stays O(1).
  ...
  python validate_one_r1.py 24            -> checks tr_batches/batch_0024.json
...
BATCHES = os.path.join(FULL_KO, "tr_batches_r1")
OUT = os.path.join(FULL_KO, "tr_out_r1")
...
def main():
    if len(sys.argv) < 2:
        raise SystemExit("usage: validate_one_r1.py <batch number>")
    n = int(sys.argv[1])
